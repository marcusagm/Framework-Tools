;(function ( $, window, undefined ) {
	...
	function nativeWindow( element, options ) {
		...
		function open() {
			...
			$.ajax({
				...
				success: function(data) {
					modal.children('div').hide().width(me.options.width).append(data);
					setSize(function() {
		...
		function setSize(callback) {
			var height;
			var contentPadding = modal.children('div').innerWidth() - modal.children('div').width();
